"""Step-wise MPC planning with delta-score rewards."""
import argparse
import glob
import os
import pathlib
...
def main(config):
    """Run step-wise MPC planning with pre-trained Dreamer world model."""
    ...
    if config.step_wise:
        ...
        goal_image_path = "logdir/planet_test/success_frame_button.png"
    ...
    agent = Dreamer(
        ...
        dummy_logger, dummy_dataset, target_embeddings[0],
    ).to(device)
    ...
    planner = MPCPlanner(
        ...
        device=device, reward_form=config.reward_form
    )
    ...
    for episode in range(1, config.eval_episodes + 1):
        ...
        plateau_counter = 0
        prev_similarity = 0.0
        timesteps, similarities, diff_rewards = [], [], []
        ...
        with torch.no_grad():
            ...
            for t in pbar:
                ...
                if config.step_wise:
                    ...
                    if plateau_counter >= config.plateau_patience:
                        if current_subtask_idx < len(target_embeddings) - 1:
                            ...
                            print(f"[{t}] Transitioning to subtask {current_subtask_idx}!")
                ...
                action_np = np.clip(to_np(action.cpu()), -1.0, 1.0)
                ...
                pbar.set_description(f"{task_info} Sim: {current_similarity:.4f}, Diff: {diff_reward:.4f}")
        ...
        plot_similarity(timesteps, similarities, diff_rewards, subtask_changes=subtask_changes, filename=plot_filename)
