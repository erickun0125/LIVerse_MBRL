...
def plot_similarity(timesteps, similarities, diff_rewards, subtask_changes=None, filename=None):
    ...
    ax2.plot(timesteps[1:], diff_rewards[1:], 'r-', label='Difference')  # Exclude first timestep
...
def load_subtasks(subtask_dir, liv, device):
    """Load subtask images and generate embeddings."""
    print(f"Loading subtasks from {subtask_dir}...")
    
    # Image transformer
    transform = T.Compose([T.ToTensor()])
    
    # Find image files (sorted by number)
...
def main(config):
    ...
    if hasattr(config, 'results_dir') and config.results_dir:
        ...
        # Default results directory name - differs based on whether step-wise mode is enabled
        if config.step_wise:
    ...
    # Target embeddings - differs based on general or step_wise mode
    target_embeddings = []
    ...
    if config.step_wise:
        ...
        goal_image_path = "logdir/planet_test/success_frame_button.png"  # Success frame path setting
        if goal_image_path and os.path.exists(goal_image_path):
            ...
            target_embeddings = [target_embedding]
            print("Generated text-based goal embedding.")
    ...
    for episode in range(1, config.eval_episodes + 1):
        ...
        # Step-wise mode variables
        current_subtask_idx = 0
        subtask_changes = []  # Record subtask change points
        plateau_counter = 0   # Counter for diff_reward plateau state

        # Variables for difference-based reward calculation
        prev_similarity = 0.0
        ...
        with torch.no_grad():
            ...
            for t in pbar:
                ...
                # Calculate similarity for next subtask
                ...
                # Check subtask completion in step-wise mode - changed to plateau detection method
                if config.step_wise:
                    # Count as plateau state if diff_reward is below threshold
                    if diff_reward < config.similarity_threshold:
                        plateau_counter += 1
                    else:
                        plateau_counter = 0  # Reset counter if above threshold
                    
                    # Transition to next subtask if plateau state persists for specified number of steps
                    if plateau_counter >= config.plateau_patience:
                        if current_subtask_idx < len(target_embeddings) - 1:
                            current_subtask_idx += 1
                            print(f"[{t}] Transitioning to subtask {current_subtask_idx}! (similarity: {current_similarity:.4f}, plateau count: {plateau_counter})")
                            subtask_changes.append((t, current_subtask_idx))
                            # Reset similarity for new subtask
                            prev_similarity = 0.0
...
if __name__ == "__main__":
    ...
    planet_specific = {
        # Set different default directory names based on step_wise (auto-numbered)
        "results_dir": "",  # Leave empty and determine internally based on step_wise mode
        "planning_horizon": 10,
        ...
        # Add step-wise related settings
        "step_wise": False,  # Whether to activate subtask mode
        "subtask_dir": "./subtasks",  # Subtask image directory
        "similarity_threshold": 0.0012,  # Similarity threshold for transitioning to next subtask
        "plateau_patience": 5  # Number of steps to wait before transitioning to next subtask
    }
